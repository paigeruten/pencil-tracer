...
unless global.testingBrowser
  ...
  test "patchStackTrace line patching", ->
    err = new Error 'error'
    ok err.stack.match /iced[\/\\]error_messages\.coffee:\d+:\d+\b/
  ...
  test "#2849: compilation error in a require()d file", ->
    ...
    path = path.join __dirname, 'syntax-error.coffee'
    ok not fs.existsSync path
    fs.writeFileSync path, 'foo in bar or in baz'

    try
      assertErrorFormat '''
        require './test/suite/iced/syntax-error'
      ''',
      """
        #{path}:1:15: error: unexpected in
        foo in bar or in baz
                      ^^
      """
    finally
